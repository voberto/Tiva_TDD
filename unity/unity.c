...
// Time library for recording current date and time in test reports (18/12/2014)
...
#define AUTHOR_STR				"Mr. Robot"
#define DATE_STR				"01jan2020"
#define PRJ_VERSION_STR			"0001"
#define COMPANY_STR				"E-Corp"
...
const char UnitySummaryStr[]				= " Test Summary: ";
...
// Time structure declaration for time variable (18/12/2014);
time_t currentTime;
...
int UnityEnd(void)
{
    ...
    UNITY_PRINT_EOL;
    trace_puts("---------------------------------------------");
    trace_puts("FINAL RESULTS");
    trace_puts("---------------------------------------------");
    //UNITY_PRINT_EOL;
    trace_fputs("NUMBER OF COMPLETED TESTS: ");
    UnityPrintNumber((_U_SINT)(Unity.NumberOfTests));
    UnityPrint(UnityStrResultsTests);
    trace_puts("");
    trace_fputs("NUMBER OF FAILURES: ");
    UnityPrintNumber((_U_SINT)(Unity.TestFailures));
    UnityPrint(UnityStrResultsFailures);
    trace_puts("");
    trace_fputs("NUMBER OF IGNORED TESTS: ");
...
//-----------------------------------------------
// Prints a manual test summary (17/12/2014)
void UnitySummary(void)
{
	trace_puts("\n");
	trace_puts("---------------------------------------------");
	trace_puts("TEST SUMMARY");
	trace_puts("");
	trace_fputs("Author: ");
	trace_fputs(AUTHOR_STR);
	trace_puts("");
	trace_fputs("Date: ");

	time(&currentTime);
	struct tm *myTime = localtime(&currentTime);		// Declares a structure pointer for manipulating time
														// components of the currentTime structure;

	trace_printf("%i/%i/%i\n", (myTime->tm_mday), (myTime->tm_mon + 1), (myTime->tm_year + 1900));
														// Prints current date in "dd/mm/yyyy" format;
	trace_fputs("Clock: ");
	trace_printf("%i:%i:%i\n", (myTime->tm_hour - 2), (myTime->tm_min), (myTime->tm_sec));
														// Prints current time in "hh:mm:ss" format;
	trace_fputs("Project version: ");
	trace_fputs(PRJ_VERSION_STR);
	trace_puts("");
	trace_fputs("Company: ");
	trace_fputs(COMPANY_STR);
	trace_puts("");
	trace_puts("---------------------------------------------");
}
